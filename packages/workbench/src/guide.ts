...
export const PANEL_CONTROLS: readonly PanelControl[] = [
  ...
  },
  {
    ...
    hint: 'Typst shared by every paper you write — a command for a claim, a style for a figure. It is read into the top of every page, so a #show or #set rule here sets them all. Saved when it compiles and refused when it does not, because a broken one would blank every page at once.',
  },
  {
    id: PANEL_CONTROL_IDS.notebookLocalHeader,
    title: 'This notebook’s own header',
    surface: 'A notebook page, in the front matter',
    hint: 'What this paper adds on top. It is read after the shared one, so it can build on what is there and a definition here quietly replaces the global one for this notebook and no other.',
  },
  {
...
export const GUIDE_CHAPTERS: readonly GuideChapter[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      { text: 'Define your own commands in the header every notebook shares, or in this notebook’s own — the local one is read second, so it can build on the shared one and wins where they disagree.' },
      { text: 'A blank line ends a block, except inside something that has not closed: a #figure or a #table written over several lines stays one block, however many blank lines are in it.' },
